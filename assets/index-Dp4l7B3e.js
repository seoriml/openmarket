(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))c(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const x of r.addedNodes)x.tagName==="LINK"&&x.rel==="modulepreload"&&c(x)}).observe(document,{childList:!0,subtree:!0});function n(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function c(o){if(o.ep)return;o.ep=!0;const r=n(o);fetch(o.href,r)}})();function B(){const e=JSON.parse(localStorage.getItem("userToken"));if(console.log(e),e&&e.user_type)switch(e.user_type){case"BUYER":return"buyer";case"SELLER":return"seller";default:return"guest"}else return"guest"}async function N(){try{if(!(await fetch(`${S}/accounts/logout/`,{method:"POST",headers:{"Content-Type":"application/json"}})).ok)throw new Error("로그아웃 요청에 실패했습니다.");localStorage.removeItem("userToken"),alert("로그아웃 되었습니다."),window.location.href="/openmarket"}catch(e){alert(e.message)}}function P(){const e=document.createElement("header");e.className="shadow-lg";const s=B(),n={guest:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#login",text:"로그인",icon:"icon-user.svg"}],buyer:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}],seller:[{href:"#seller-center",text:"판매자 센터",icon:"icon-shop.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}]},c=({href:l,text:p,icon:u})=>`
  ...
  `;const c=e.querySelector("button.buyer"),o=e.querySelector("button.seller"),r=e.querySelector(".user-form");let x=c,g="BUYER";const b=(l,p)=>{const u=p.getAttribute("data-type").toLowerCase(),f=l.getAttribute("data-type");p.classList.contains("active")&&p.classList.remove("active"),!l.classList.contains("active")&&l.classList.add("active"),g=f,r.classList.contains(u)&&r.classList.remove(u),!r.classList.contains(f.toLowerCase())&&r.classList.add(f.toLowerCase()),x=l};return c.addEventListener("click",()=>{b(c,x)}),o.addEventListener("click",()=>{b(o,x)}),r.addEventListener("submit",async l=>{l.preventDefault();const p=e.querySelector("#userId").value.trim(),u=e.querySelector("#password").value.trim(),f=e.querySelector("#loginError");let h="";if(p===""&&u===""?h="아이디와 비밀번호를 입력해주세요.":p===""?h="아이디를 입력해주세요.":u===""&&(h="비밀번호를 입력해주세요."),h){f.textContent=h,f.style.display="block",p===""?e.querySelector("#userId").focus():u===""&&e.querySelector("#password").focus();return}const w=await fetch("https://openmarket.weniv.co.kr/accounts/login/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:p,password:u,login_type:g})});if(w.ok){const $=await w.json(),y={user_type:$.user_type,token:$.token,cart:[]};localStorage.setItem("userToken",JSON.stringify(y));const q=sessionStorage.getItem("beforePage");window.location.href=`/openmarket/${q}`}else f.textContent="아이디 또는 비밀번호가 일치하지 않습니다.",f.style.display="block",u.value="",u.focus()}),e}async function O(){const e=JSON.parse(localStorage.getItem("userToken")),s={async fetch(t,a={}){const d={headers:{"Content-Type":"application/json",...e&&{Authorization:`JWT ${e.token}`}}},i=await fetch(`${S}${t}`,{...d,...a});if(!i.ok)throw new Error(`API request failed: ${i.statusText}`);return i.json()},getCart:()=>s.fetch("/cart/"),getProduct:t=>s.fetch(`/products/${t}/`),updateCartItem:(t,a)=>s.fetch(`/cart/${t}/`,{method:"PUT",body:JSON.stringify(a)}),deleteCartItem:async t=>{const a=await fetch(`${S}/cart/${t}/`,{method:"DELETE",headers:{Authorization:`JWT ${e.token}`}});if(!a.ok)throw new Error(`API request failed: ${a.statusText}`);return a.status===204?null:a.json()}},n=document.createElement("section");n.className="max-w-container m-auto text-center";const c=await s.getCart();let o=await Promise.all(c.results.map(async t=>{const a=await s.getProduct(t.product_id);return{...t,product:a}}));function r(){return o.map(t=>`
      ...
      `).join("")}const x="w-1/4",g="w-[34px] h-[34px] bg-white bg-contain bg-center bg-no-repeat rounded-full",b="mb-[12px",l="text-[24px] font-bold",p="modal fixed inset-0 bg-black bg-opacity-50 hidden z-50 ",u="border border-[#c4c4c4] py-[10px] w-[100px] rounded-[5px]",f="top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-[360px] text-center relative bg-white pt-[50px] pb-[40px] border border-[#767676]",h="w-[50px] h-[50px] bg-contain bg-center bg-no-repeat",w="absolute top-[18px] right-[18px] w-[22px] h-[22px] bg-[url('/images/icon-delete.svg')] bg-no-repeat bg-center bg-contain",$="w-[50px] h-[50px] border-x-[1px] border-[#c4c4c4] flex items-center justify-center text-center text-[18px] focus:outline-none";n.innerHTML=`
  ...
  <div id="quantity-modal" class="${p}">
    <div class="${f} absolute flex flex-col items-center">
      ...
      <div class="w-[150px] border border-[#c4c4c4] rounded-[10px] flex items-center justify-center">
        ...
        <input type="number" id="quantity-input" min="1" class="${$}" />
...
`;const y=()=>{let t=0,a=0;o.forEach(d=>{n.querySelector(`div[data-id="${d.cart_item_id}"] .item-checkbox`).checked&&(t+=d.product.price*d.quantity,a+=d.product.shipping_fee)}),n.querySelector("#total-price").textContent=`${t.toLocaleString()}`,n.querySelector("#shipping-fee").textContent=`${a.toLocaleString()}`,n.querySelector("#final-price").textContent=`${(t+a).toLocaleString()}`},q=async(t,a)=>{const d=o.find(i=>i.cart_item_id===t);if(a>d.product.stock){alert("재고 수량을 초과했습니다.");return}try{await s.updateCartItem(t,{product_id:d.product.product_id,quantity:a,is_active:!0}),d.quantity=a;const i=n.querySelector(`div[data-id="${t}"]`);if(i){const m=i.querySelector(".quantity"),v=i.querySelector(".item-total-price");m&&(m.textContent=a),v?v.innerHTML=`
            ${(d.product.price*a).toLocaleString()} 원
          `:console.error("가격 표시 요소가 없습니다."),y()}else console.error("장바구니 항목을 찾을 수 없습니다.")}catch(i){console.error("수량 업데이트 실패:",i),alert("수량 업데이트에 실패했습니다.")}},_=async t=>{try{const a=await s.deleteCartItem(t);o=o.filter(i=>i.cart_item_id!==t);const d=n.querySelector(`div[data-id="${t}"]`);d&&d.remove(),y()}catch{alert("상품 삭제에 실패했습니다.")}},E=(t,a,d)=>{const i=n.querySelector("#quantity-modal"),m=i.querySelector("#quantity-input");m.value=a,m.max=d,i.style.display="block",i.querySelector("#increase-quantity").onclick=()=>{const v=parseInt(m.value)+1;v<=d&&(m.value=v)},i.querySelector("#decrease-quantity").onclick=()=>{const v=parseInt(m.value)-1;v>=1&&(m.value=v)},m.oninput=()=>{const v=parseInt(m.value);isNaN(v)||v<1?m.value=1:v>d&&(m.value=d)},i.querySelector("#quantity-confirm").onclick=()=>{q(t,parseInt(m.value)),i.style.display="none"},i.querySelector("#quantity-cancel").onclick=()=>{i.style.display="none"},i.querySelector("#quantity-close").onclick=()=>{i.style.display="none"}},T=t=>{const a=n.querySelector("#delete-modal");a.style.display="block",a.querySelector("#delete-confirm").onclick=async()=>{await _(t),a.style.display="none"},a.querySelector("#delete-cancel").onclick=()=>{a.style.display="none"},a.querySelector("#delete-close").onclick=()=>{a.style.display="none"}};return n.querySelector(".cart-items").addEventListener("click",t=>{const a=t.target.closest("div[data-id]");if(!a)return;const d=parseInt(a.dataset.id),i=parseInt(a.querySelector(".quantity").textContent),m=parseInt(a.dataset.stock);t.target.classList.contains("increase-quantity")||t.target.classList.contains("decrease-quantity")?E(d,i,m):t.target.classList.contains("delete-item")?T(d):t.target.classList.contains("item-checkbox")&&y()}),n.querySelector("#select-all").addEventListener("change",t=>{n.querySelectorAll(".item-checkbox").forEach(a=>a.checked=t.target.checked),y()}),y(),n}async function H(){try{const e=await fetch(`${S}/products/`);if(!e.ok)throw new Error("상품 데이터를 가져오는 데 실패했습니다.");return(await e.json()).results}catch(e){return console.error(e),[]}}async function A(){const e=document.createElement("section");e.className="product-list-page max-w-container m-auto py-[80px]";const s=await H();if(s.length===0)return e.innerHTML="<p>상품이 없습니다.</p>",e;console.log(s);const n=s.map(c=>`
  ...
  `,e}async function D(e){try{const s=await fetch(`${S}/products/${e}/`);if(!s.ok)throw new Error("상품 상세 데이터를 가져오는 데 실패했습니다.");return await s.json()}catch(s){throw console.error(s),s}}async function F(e){const s=document.createElement("div");try{const n=await D(e);s.className="product-detail max-w-container m-auto py-[80px]",s.innerHTML=`
      ...
      `}catch(n){console.error("상품 상세 정보를 가져오는 데 실패했습니다.",n)}return s}function J(){const e=["https://via.placeholder.com/1200x500?text=Banner+1","https://via.placeholder.com/1200x500?text=Banner+2","https://via.placeholder.com/1200x500?text=Banner+3"];let s=0,n;function c(){const o=document.createElement("div");o.className="relative w-full h-full mx-auto bg-[#f2f2f2] max-h-[500px] overflow-hidden";const r=document.createElement("img");r.src=e[s],r.alt=`배너 ${s+1}`,r.className="w-full h-auto transition-opacity duration-500",o.appendChild(r);const x="absolute top-1/2 transform -translate-y-1/2 text-white px-4 py-2 focus:outline-none w-[8px] h-[16px] bg-no-repeat bg-center",g=document.createElement("button");g.className=`${x} bg-[url('/images/icon-swiper-1.svg')] left-4`,g.addEventListener("click",()=>{s=(s-1+e.length)%e.length,p(),h()});const b=document.createElement("button");b.className=`${x} bg-[url('/images/icon-swiper-2.svg')] right-4`,b.addEventListener("click",()=>{s=(s+1)%e.length,p(),h()}),o.appendChild(g),o.appendChild(b);const l=document.createElement("div");l.className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2",e.forEach((w,$)=>{const y=document.createElement("span");y.className=`w-[6px] h-[6px] rounded-full cursor-pointer transition-colors duration-300 ${$===s?"bg-black":"bg-white"}`,y.addEventListener("click",()=>{s=$,p(),h()}),l.appendChild(y)}),o.appendChild(l);function p(){r.src=e[s],r.alt=`배너 ${s+1}`,u()}function u(){l.querySelectorAll("span").forEach(($,y)=>{$.className=`w-2 h-2 rounded-full cursor-pointer transition-colors duration-300 ${y===s?"bg-black":"bg-white"}`})}function f(){n=setInterval(()=>{s=(s+1)%e.length,p()},3e3)}function h(){clearInterval(n),f()}return f(),o}return c()}const S="https://openmarket.weniv.co.kr",Y={login:z,cart:O,home:A,detail:F};async function I(){const e=document.getElementById("app");let s=!1,n="",c=window.location.hash.slice(1);c.includes("/")&&(n=c.split("/")[1],c=c.split("/")[0],s=!0),e.innerHTML="";try{const o=Y[c]||NotFound,r=s?await o(n):await o();if(c!=="login"){const g=P();e.appendChild(g)}if(c==="home"){const g=J();e.appendChild(g)}e.appendChild(r);const x=M();e.appendChild(x)}catch(o){console.error("페이지 렌더링 중 오류 발생:",o),e.innerHTML="<p>페이지를 로드하는 중 오류가 발생했습니다.</p>"}}function R(){window.addEventListener("hashchange",I),window.location.hash?I():window.location.hash="home"}window.addEventListener("load",R);
