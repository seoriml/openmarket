(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))c(r);new MutationObserver(r=>{for(const l of r)if(l.type==="childList")for(const p of l.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&c(p)}).observe(document,{childList:!0,subtree:!0});function a(r){const l={};return r.integrity&&(l.integrity=r.integrity),r.referrerPolicy&&(l.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?l.credentials="include":r.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function c(r){if(r.ep)return;r.ep=!0;const l=a(r);fetch(r.href,l)}})();function j(){const e=JSON.parse(localStorage.getItem("userToken"));if(console.log(e),e&&e.user_type)switch(e.user_type){case"BUYER":return"buyer";case"SELLER":return"seller";default:return"guest"}else return"guest"}async function P(){try{if(!(await fetch(`${L}/accounts/logout/`,{method:"POST",headers:{"Content-Type":"application/json"}})).ok)throw new Error("로그아웃 요청에 실패했습니다.");localStorage.removeItem("userToken"),alert("로그아웃 되었습니다."),window.location.href="/openmarket"}catch(e){alert(e.message)}}function B(){const e=document.createElement("header");e.className="shadow-lg";const o=j(),a={guest:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#login",text:"로그인",icon:"icon-user.svg"}],buyer:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}],seller:[{href:"#seller-center",text:"판매자 센터",icon:"icon-shop.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}]},c=({href:d,text:u,icon:x})=>`
  ...
  `,o==="guest"){const d=e.querySelector('a[href="/openmarket/#cart"]');d&&d.addEventListener("click",u=>{u.preventDefault();const x=document.getElementById("loginModal");x.classList.remove("hidden");const f=document.getElementById("modalYes"),g=document.getElementById("modalNo"),h=document.getElementById("modalClose");f.addEventListener("click",()=>{sessionStorage.setItem("beforePage",window.location.hash),window.location.hash="login"}),g.addEventListener("click",()=>{x.classList.add("hidden")}),h.addEventListener("click",()=>{x.classList.add("hidden")})})}const l=e.querySelector('a[href="/openmarket/#login"]');l&&l.addEventListener("click",d=>{d.preventDefault(),sessionStorage.setItem("beforePage",window.location.hash),window.location.hash="login"});const p=e.querySelector('a[href="/openmarket/#logout"]');return p&&p.addEventListener("click",d=>{d.preventDefault(),P()}),e.querySelector(".search-form").addEventListener("submit",d=>{d.preventDefault();const u=e.querySelector("#search-input");console.log("검색어:",u.value)}),e}const M="bg-[#f2f2f2] py-[60px]",N="mb-[30px] flex justify-between border-b-2",z="flex space-x-[14px] text-sm mb-[30px]",k="border-r border-[#c4c4c4] pr-[14px] last:border-none",w="hover:font-bold",C="h-6 w-6",O="text-sm text-[#767676] leading-[24px]",H="font-semibold";function D(){const e=document.createElement("footer");return e.className=M,e.innerHTML=`
  ...
  `;const c=e.querySelector("button.buyer"),r=e.querySelector("button.seller"),l=e.querySelector(".user-form");let p=c,y="BUYER";const d=(u,x)=>{const f=x.getAttribute("data-type").toLowerCase(),g=u.getAttribute("data-type");x.classList.contains("active")&&x.classList.remove("active"),!u.classList.contains("active")&&u.classList.add("active"),y=g,l.classList.contains(f)&&l.classList.remove(f),!l.classList.contains(g.toLowerCase())&&l.classList.add(g.toLowerCase()),p=u};return c.addEventListener("click",()=>{d(c,p)}),r.addEventListener("click",()=>{d(r,p)}),l.addEventListener("submit",async u=>{u.preventDefault();const x=e.querySelector("#userId").value.trim(),f=e.querySelector("#password").value.trim(),g=e.querySelector("#loginError");let h="";if(x===""&&f===""?h="아이디와 비밀번호를 입력해주세요.":x===""?h="아이디를 입력해주세요.":f===""&&(h="비밀번호를 입력해주세요."),h){g.textContent=h,g.style.display="block",x===""?e.querySelector("#userId").focus():f===""&&e.querySelector("#password").focus();return}const $=await fetch("https://openmarket.weniv.co.kr/accounts/login/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:x,password:f,login_type:y})});if($.ok){const S=await $.json(),v={user_type:S.user_type,token:S.token,cart:[]};localStorage.setItem("userToken",JSON.stringify(v));const q=sessionStorage.getItem("beforePage");window.location.href=`/openmarket/${q}`}else g.textContent="아이디 또는 비밀번호가 일치하지 않습니다.",g.style.display="block",f.value="",f.focus()}),e}async function F(){const e=JSON.parse(localStorage.getItem("userToken")),o={async fetch(t,s={}){const i={headers:{"Content-Type":"application/json",...e&&{Authorization:`JWT ${e.token}`}}},n=await fetch(`${L}${t}`,{...i,...s});if(!n.ok)throw new Error(`API request failed: ${n.statusText}`);return n.json()},getCart:()=>o.fetch("/cart/"),getProduct:t=>o.fetch(`/products/${t}/`),updateCartItem:(t,s)=>o.fetch(`/cart/${t}/`,{method:"PUT",body:JSON.stringify(s)}),deleteCartItem:async t=>{const s=await fetch(`${L}/cart/${t}/`,{method:"DELETE",headers:{Authorization:`JWT ${e.token}`}});if(!s.ok)throw new Error(`API request failed: ${s.statusText}`);return s.status===204?null:s.json()}},a=document.createElement("section");a.className="max-w-container m-auto text-center";const c=await o.getCart();let r=await Promise.all(c.results.map(async t=>{const s=await o.getProduct(t.product_id);return{...t,product:s}}));function l(){return r.map(t=>`
        <!-- 장바구니 항목 시작 -->
      ...
      `).join("")}const p="w-1/4",y="w-[34px] h-[34px] bg-white bg-contain bg-center bg-no-repeat rounded-full",d="mb-[12px",u="text-[24px] font-bold",x="modal fixed inset-0 bg-black bg-opacity-50 hidden",f="border border-[#c4c4c4] py-[10px] w-[100px] rounded-[5px]",g="top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-[360px] text-center relative bg-white pt-[50px] pb-[40px] border border-[#767676]",h="w-[50px] h-[50px] bg-contain bg-center bg-no-repeat",$="absolute top-[18px] right-[18px] w-[22px] h-[22px] bg-[url('/images/icon-delete.svg')] bg-no-repeat bg-center bg-contain",S="w-[50px] h-[50px] border-x-[1px] border-[#c4c4c4] flex items-center justify-center text-center text-[18px] focus:outline-none";a.innerHTML=`
  ...
  <div id="quantity-modal" class="${x}">
    <div class="${g} absolute flex flex-col items-center">
      <button id="modalClose" class="${$}"></button> 
      <div class="w-[150px] border border-[#c4c4c4] rounded-[10px] flex items-center justify-center">
        ...
        <input type="number" id="quantity-input" min="1" class="${S}" />
...
`;const v=()=>{let t=0,s=0;r.forEach(i=>{a.querySelector(`div[data-id="${i.cart_item_id}"] .item-checkbox`).checked&&(t+=i.product.price*i.quantity,s+=i.product.shipping_fee)}),a.querySelector("#total-price").textContent=`${t.toLocaleString()}`,a.querySelector("#shipping-fee").textContent=`${s.toLocaleString()}`,a.querySelector("#final-price").textContent=`${(t+s).toLocaleString()}`},q=async(t,s)=>{const i=r.find(n=>n.cart_item_id===t);if(s>i.product.stock){alert("재고 수량을 초과했습니다.");return}try{await o.updateCartItem(t,{product_id:i.product.product_id,quantity:s,is_active:!0}),i.quantity=s;const n=a.querySelector(`div[data-id="${t}"]`);if(n){const m=n.querySelector(".quantity"),b=n.querySelector(".item-total-price");m&&(m.textContent=s),b?b.innerHTML=`
  ...
  `,e}async function R(){try{const e=await fetch(`${L}/products/`);if(!e.ok)throw new Error("상품 데이터를 가져오는 데 실패했습니다.");return(await e.json()).results}catch(e){return console.error(e),[]}}const L="https://openmarket.weniv.co.kr",V={home:U,login:A,cart:F,detail:Y};async function I(){const e=document.getElementById("app");let o=!1,a="",c=window.location.hash.slice(1);c.includes("/")&&(a=c.split("/")[1],c=c.split("/")[0],o=!0),e.innerHTML="";try{const r=V[c]||NotFound,l=o?await r(a):await r();if(c!=="login"){const y=B();e.appendChild(y)}e.appendChild(l);const p=D();e.appendChild(p)}catch(r){console.error("페이지 렌더링 중 오류 발생:",r),e.innerHTML="<p>페이지를 로드하는 중 오류가 발생했습니다.</p>"}}function W(){window.addEventListener("hashchange",I),window.location.hash?I():window.location.hash="home"}window.addEventListener("load",W);
