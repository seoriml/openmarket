(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))c(n);new MutationObserver(n=>{for(const l of n)if(l.type==="childList")for(const d of l.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&c(d)}).observe(document,{childList:!0,subtree:!0});function a(n){const l={};return n.integrity&&(l.integrity=n.integrity),n.referrerPolicy&&(l.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?l.credentials="include":n.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function c(n){if(n.ep)return;n.ep=!0;const l=a(n);fetch(n.href,l)}})();function j(){const e=JSON.parse(localStorage.getItem("userToken"));if(console.log(e),e&&e.user_type)switch(e.user_type){case"BUYER":return"buyer";case"SELLER":return"seller";default:return"guest"}else return"guest"}async function P(){try{if(!(await fetch(`${L}/accounts/logout/`,{method:"POST",headers:{"Content-Type":"application/json"}})).ok)throw new Error("로그아웃 요청에 실패했습니다.");localStorage.removeItem("userToken"),alert("로그아웃 되었습니다."),window.location.href="/openmarket"}catch(e){alert(e.message)}}function B(){const e=document.createElement("header");e.className="shadow-lg";const o=j(),a={guest:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#login",text:"로그인",icon:"icon-user.svg"}],buyer:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}],seller:[{href:"#seller-center",text:"판매자 센터",icon:"icon-shop.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}]},c=({href:p,text:u,icon:x})=>`
  ...
  `,n="border border-[#c4c4c4] py-[10px] w-[100px] rounded-[5px]";if(e.innerHTML=`
    ...
    <div id="loginModal" class="modal delete-modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
      <div class="modal-content bg-white pt-[50px] pb-[40px] border border-[#767676] w-full max-w-[360px] text-center relative">
        ...
        <div class="flex justify-center gap-[10px] mt-[30px]">
          <button id="modalNo" class="${n}  text-[#767676]">아니요</button>
          <button id="modalYes" class="${n}] bg-primary text-white">예</button>
        </div>
      </div>
    </div>
  ...
  `;const c=e.querySelector("button.buyer"),n=e.querySelector("button.seller"),l=e.querySelector(".user-form");let d=c,y="BUYER";const p=(u,x)=>{const f=x.getAttribute("data-type").toLowerCase(),g=u.getAttribute("data-type");x.classList.contains("active")&&x.classList.remove("active"),!u.classList.contains("active")&&u.classList.add("active"),y=g,l.classList.contains(f)&&l.classList.remove(f),!l.classList.contains(g.toLowerCase())&&l.classList.add(g.toLowerCase()),d=u};return c.addEventListener("click",()=>{p(c,d)}),n.addEventListener("click",()=>{p(n,d)}),l.addEventListener("submit",async u=>{u.preventDefault();const x=e.querySelector("#userId").value.trim(),f=e.querySelector("#password").value.trim(),g=e.querySelector("#loginError");let h="";if(x===""&&f===""?h="아이디와 비밀번호를 입력해주세요.":x===""?h="아이디를 입력해주세요.":f===""&&(h="비밀번호를 입력해주세요."),h){g.textContent=h,g.style.display="block",x===""?e.querySelector("#userId").focus():f===""&&e.querySelector("#password").focus();return}const $=await fetch("https://openmarket.weniv.co.kr/accounts/login/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:x,password:f,login_type:y})});if($.ok){const q=await $.json(),v={user_type:q.user_type,token:q.token,cart:[]};localStorage.setItem("userToken",JSON.stringify(v));const S=sessionStorage.getItem("beforePage");window.location.href=`/openmarket/${S}`}else g.textContent="아이디 또는 비밀번호가 일치하지 않습니다.",g.style.display="block",f.value="",f.focus()}),e}async function F(){const e=JSON.parse(localStorage.getItem("userToken")),o={async fetch(t,s={}){const i={headers:{"Content-Type":"application/json",...e&&{Authorization:`JWT ${e.token}`}}},r=await fetch(`${L}${t}`,{...i,...s});if(!r.ok)throw new Error(`API request failed: ${r.statusText}`);return r.json()},getCart:()=>o.fetch("/cart/"),getProduct:t=>o.fetch(`/products/${t}/`),updateCartItem:(t,s)=>o.fetch(`/cart/${t}/`,{method:"PUT",body:JSON.stringify(s)}),deleteCartItem:async t=>{const s=await fetch(`${L}/cart/${t}/`,{method:"DELETE",headers:{Authorization:`JWT ${e.token}`}});if(!s.ok)throw new Error(`API request failed: ${s.statusText}`);return s.status===204?null:s.json()}},a=document.createElement("section");a.className="max-w-container m-auto text-center";const c=await o.getCart();let n=await Promise.all(c.results.map(async t=>{const s=await o.getProduct(t.product_id);return{...t,product:s}}));function l(){return n.map(t=>`
        <!-- 장바구니 항목 시작 -->
...
`;const v=()=>{let t=0,s=0;n.forEach(i=>{a.querySelector(`div[data-id="${i.cart_item_id}"] .item-checkbox`).checked&&(t+=i.product.price*i.quantity,s+=i.product.shipping_fee)}),a.querySelector("#total-price").textContent=`${t.toLocaleString()}`,a.querySelector("#shipping-fee").textContent=`${s.toLocaleString()}`,a.querySelector("#final-price").textContent=`${(t+s).toLocaleString()}`},S=async(t,s)=>{const i=n.find(r=>r.cart_item_id===t);if(s>i.product.stock){alert("재고 수량을 초과했습니다.");return}try{await o.updateCartItem(t,{product_id:i.product.product_id,quantity:s,is_active:!0}),i.quantity=s;const r=a.querySelector(`div[data-id="${t}"]`);if(r){const m=r.querySelector(".quantity"),b=r.querySelector(".item-total-price");m&&(m.textContent=s),b?b.innerHTML=`
            ${(i.product.price*s).toLocaleString()} 원
          `:console.error("가격 표시 요소가 없습니다."),v()}else console.error("장바구니 항목을 찾을 수 없습니다.")}catch(r){console.error("수량 업데이트 실패:",r),alert("수량 업데이트에 실패했습니다.")}},_=async t=>{try{const s=await o.deleteCartItem(t);n=n.filter(r=>r.cart_item_id!==t);const i=a.querySelector(`div[data-id="${t}"]`);i&&i.remove(),v()}catch{alert("상품 삭제에 실패했습니다.")}},E=(t,s,i)=>{const r=a.querySelector("#quantity-modal"),m=r.querySelector("#quantity-input");m.value=s,m.max=i,r.style.display="block",r.querySelector("#increase-quantity").onclick=()=>{const b=parseInt(m.value)+1;b<=i&&(m.value=b)},r.querySelector("#decrease-quantity").onclick=()=>{const b=parseInt(m.value)-1;b>=1&&(m.value=b)},m.oninput=()=>{const b=parseInt(m.value);isNaN(b)||b<1?m.value=1:b>i&&(m.value=i)},r.querySelector("#quantity-confirm").onclick=()=>{S(t,parseInt(m.value)),r.style.display="none"},r.querySelector("#quantity-cancel").onclick=()=>{r.style.display="none"},r.querySelector("#quantity-close").onclick=()=>{r.style.display="none"}},T=t=>{const s=a.querySelector("#delete-modal");s.style.display="block",s.querySelector("#delete-confirm").onclick=async()=>{await _(t),s.style.display="none"},s.querySelector("#delete-cancel").onclick=()=>{s.style.display="none"},s.querySelector("#delete-close").onclick=()=>{s.style.display="none"}};return a.querySelector(".cart-items").addEventListener("click",t=>{const s=t.target.closest("div[data-id]");if(!s)return;const i=parseInt(s.dataset.id),r=parseInt(s.querySelector(".quantity").textContent),m=parseInt(s.dataset.stock);t.target.classList.contains("increase-quantity")||t.target.classList.contains("decrease-quantity")&&r>1?E(i,r,m):t.target.classList.contains("delete-item")?T(i):t.target.classList.contains("item-checkbox")&&v()}),a.querySelector("#select-all").addEventListener("change",t=>{a.querySelectorAll(".item-checkbox").forEach(s=>s.checked=t.target.checked),v()}),v(),a}async function J(e){try{const o=await fetch(`${L}/products/${e}/`);if(!o.ok)throw new Error("상품 상세 데이터를 가져오는 데 실패했습니다.");return await o.json()}catch(o){throw console.error(o),o}}async function Y(e){const o=document.createElement("div");try{const a=await J(e);o.className="product-detail max-w-container m-auto py-[80px]",o.innerHTML=`
  ...
  `,e}async function R(){try{const e=await fetch(`${L}/products/`);if(!e.ok)throw new Error("상품 데이터를 가져오는 데 실패했습니다.");return(await e.json()).results}catch(e){return console.error(e),[]}}const L="https://openmarket.weniv.co.kr",V={home:U,login:A,cart:F,detail:Y};async function I(){const e=document.getElementById("app");let o=!1,a="",c=window.location.hash.slice(1);c.includes("/")&&(a=c.split("/")[1],c=c.split("/")[0],o=!0),e.innerHTML="";try{const n=V[c]||NotFound,l=o?await n(a):await n();if(c!=="login"){const y=B();e.appendChild(y)}e.appendChild(l);const d=D();e.appendChild(d)}catch(n){console.error("페이지 렌더링 중 오류 발생:",n),e.innerHTML="<p>페이지를 로드하는 중 오류가 발생했습니다.</p>"}}function W(){window.addEventListener("hashchange",I),window.location.hash?I():window.location.hash="home"}window.addEventListener("load",W);
