(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))l(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const x of r.addedNodes)x.tagName==="LINK"&&x.rel==="modulepreload"&&l(x)}).observe(document,{childList:!0,subtree:!0});function a(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function l(o){if(o.ep)return;o.ep=!0;const r=a(o);fetch(o.href,r)}})();function B(){const e=JSON.parse(localStorage.getItem("userToken"));if(console.log(e),e&&e.user_type)switch(e.user_type){case"BUYER":return"buyer";case"SELLER":return"seller";default:return"guest"}else return"guest"}async function N(){try{if(!(await fetch(`${S}/accounts/logout/`,{method:"POST",headers:{"Content-Type":"application/json"}})).ok)throw new Error("로그아웃 요청에 실패했습니다.");localStorage.removeItem("userToken"),alert("로그아웃 되었습니다."),window.location.href="/openmarket"}catch(e){alert(e.message)}}function P(){const e=document.createElement("header");e.className="shadow-lg";const s=B(),a={guest:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#login",text:"로그인",icon:"icon-user.svg"}],buyer:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}],seller:[{href:"#seller-center",text:"판매자 센터",icon:"icon-shop.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}]},l=({href:i,text:d,icon:u})=>`
  ...
  `;const l=e.querySelector("button.buyer"),o=e.querySelector("button.seller"),r=e.querySelector(".user-form");let x=l,b="BUYER";const i=(d,u)=>{const m=u.getAttribute("data-type").toLowerCase(),g=d.getAttribute("data-type");u.classList.contains("active")&&u.classList.remove("active"),!d.classList.contains("active")&&d.classList.add("active"),b=g,r.classList.contains(m)&&r.classList.remove(m),!r.classList.contains(g.toLowerCase())&&r.classList.add(g.toLowerCase()),x=d};return l.addEventListener("click",()=>{i(l,x)}),o.addEventListener("click",()=>{i(o,x)}),r.addEventListener("submit",async d=>{d.preventDefault();const u=e.querySelector("#userId").value.trim(),m=e.querySelector("#password").value.trim(),g=e.querySelector("#loginError");let h="";if(u===""&&m===""?h="아이디와 비밀번호를 입력해주세요.":u===""?h="아이디를 입력해주세요.":m===""&&(h="비밀번호를 입력해주세요."),h){g.textContent=h,g.style.display="block",u===""?e.querySelector("#userId").focus():m===""&&e.querySelector("#password").focus();return}const $=await fetch("https://openmarket.weniv.co.kr/accounts/login/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,password:m,login_type:b})});if($.ok){const w=await $.json(),y={user_type:w.user_type,token:w.token,cart:[]};localStorage.setItem("userToken",JSON.stringify(y));const q=sessionStorage.getItem("beforePage");window.location.href=`/openmarket/${q}`}else g.textContent="아이디 또는 비밀번호가 일치하지 않습니다.",g.style.display="block",m.value="",m.focus()}),e}async function O(){const e=JSON.parse(localStorage.getItem("userToken")),s={async fetch(t,n={}){const p={headers:{"Content-Type":"application/json",...e&&{Authorization:`JWT ${e.token}`}}},c=await fetch(`${S}${t}`,{...p,...n});if(!c.ok)throw new Error(`API request failed: ${c.statusText}`);return c.json()},getCart:()=>s.fetch("/cart/"),getProduct:t=>s.fetch(`/products/${t}/`),updateCartItem:(t,n)=>s.fetch(`/cart/${t}/`,{method:"PUT",body:JSON.stringify(n)}),deleteCartItem:async t=>{const n=await fetch(`${S}/cart/${t}/`,{method:"DELETE",headers:{Authorization:`JWT ${e.token}`}});if(!n.ok)throw new Error(`API request failed: ${n.statusText}`);return n.status===204?null:n.json()}},a=document.createElement("section");a.className="max-w-container m-auto text-center";const l=await s.getCart();let o=await Promise.all(l.results.map(async t=>{const n=await s.getProduct(t.product_id);return{...t,product:n}}));function r(){return o.map(t=>`
        <!-- 장바구니 항목 시작 -->
      ...
      `).join("")}const x="w-1/4",b="w-[34px] h-[34px] bg-white bg-contain bg-center bg-no-repeat rounded-full",i="mb-[12px",d="text-[24px] font-bold",u="modal fixed inset-0 bg-black bg-opacity-50 hidden",m="border border-[#c4c4c4] py-[10px] w-[100px] rounded-[5px]",g="top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-[360px] text-center relative bg-white pt-[50px] pb-[40px] border border-[#767676]",h="w-[50px] h-[50px] bg-contain bg-center bg-no-repeat",$="absolute top-[18px] right-[18px] w-[22px] h-[22px] bg-[url('/images/icon-delete.svg')] bg-no-repeat bg-center bg-contain",w="w-[50px] h-[50px] border-x-[1px] border-[#c4c4c4] flex items-center justify-center text-center text-[18px] focus:outline-none";a.innerHTML=`
...
`;const y=()=>{let t=0,n=0;o.forEach(p=>{a.querySelector(`div[data-id="${p.cart_item_id}"] .item-checkbox`).checked&&(t+=p.product.price*p.quantity,n+=p.product.shipping_fee)}),a.querySelector("#total-price").textContent=`${t.toLocaleString()}`,a.querySelector("#shipping-fee").textContent=`${n.toLocaleString()}`,a.querySelector("#final-price").textContent=`${(t+n).toLocaleString()}`},q=async(t,n)=>{const p=o.find(c=>c.cart_item_id===t);if(n>p.product.stock){alert("재고 수량을 초과했습니다.");return}try{await s.updateCartItem(t,{product_id:p.product.product_id,quantity:n,is_active:!0}),p.quantity=n;const c=a.querySelector(`div[data-id="${t}"]`);if(c){const f=c.querySelector(".quantity"),v=c.querySelector(".item-total-price");f&&(f.textContent=n),v?v.innerHTML=`
            ${(p.product.price*n).toLocaleString()} 원
          `:console.error("가격 표시 요소가 없습니다."),y()}else console.error("장바구니 항목을 찾을 수 없습니다.")}catch(c){console.error("수량 업데이트 실패:",c),alert("수량 업데이트에 실패했습니다.")}},_=async t=>{try{const n=await s.deleteCartItem(t);o=o.filter(c=>c.cart_item_id!==t);const p=a.querySelector(`div[data-id="${t}"]`);p&&p.remove(),y()}catch{alert("상품 삭제에 실패했습니다.")}},E=(t,n,p)=>{const c=a.querySelector("#quantity-modal"),f=c.querySelector("#quantity-input");f.value=n,f.max=p,c.style.display="block",c.querySelector("#increase-quantity").onclick=()=>{const v=parseInt(f.value)+1;v<=p&&(f.value=v)},c.querySelector("#decrease-quantity").onclick=()=>{const v=parseInt(f.value)-1;v>=1&&(f.value=v)},f.oninput=()=>{const v=parseInt(f.value);isNaN(v)||v<1?f.value=1:v>p&&(f.value=p)},c.querySelector("#quantity-confirm").onclick=()=>{q(t,parseInt(f.value)),c.style.display="none"},c.querySelector("#quantity-cancel").onclick=()=>{c.style.display="none"},c.querySelector("#quantity-close").onclick=()=>{c.style.display="none"}},T=t=>{const n=a.querySelector("#delete-modal");n.style.display="block",n.querySelector("#delete-confirm").onclick=async()=>{await _(t),n.style.display="none"},n.querySelector("#delete-cancel").onclick=()=>{n.style.display="none"},n.querySelector("#delete-close").onclick=()=>{n.style.display="none"}};return a.querySelector(".cart-items").addEventListener("click",t=>{const n=t.target.closest("div[data-id]");if(!n)return;const p=parseInt(n.dataset.id),c=parseInt(n.querySelector(".quantity").textContent),f=parseInt(n.dataset.stock);t.target.classList.contains("increase-quantity")||t.target.classList.contains("decrease-quantity")&&c>1?E(p,c,f):t.target.classList.contains("delete-item")?T(p):t.target.classList.contains("item-checkbox")&&y()}),a.querySelector("#select-all").addEventListener("change",t=>{a.querySelectorAll(".item-checkbox").forEach(n=>n.checked=t.target.checked),y()}),y(),a}async function H(){try{const e=await fetch(`${S}/products/`);if(!e.ok)throw new Error("상품 데이터를 가져오는 데 실패했습니다.");return(await e.json()).results}catch(e){return console.error(e),[]}}async function D(){const e=document.createElement("section");e.className="product-list-page max-w-container m-auto py-[80px]";const s=await H();if(s.length===0)return e.innerHTML="<p>상품이 없습니다.</p>",e;console.log(s);const a=s.map(l=>`
        <li data-product-id="${l.product_id}" class="product-item">
          <a href="/openmarket/#detail/${l.product_id}">
            <image src="${l.image}" alt="${l.product_name}" class="w-full h-full max-w-[380px] max-h-[380px] object-cover border border-[#c4c4c4] rounded-[10px]">
          <p class="text-[16px] text-[#767676] mt-[16px]">${l.store_name.replace(/\x08/g,"")}</p>
          <h2 class="text-[18px] mt-[10px]">${l.product_name}</h2>
          <p class="text-[18px] mt-[10px]"><b class="text-[24px] font-bold">${l.price.toLocaleString()}</b> 원</p>
          </a>
        </li>
        `).join("");return e.innerHTML=`
    <ul class="products grid grid-cols-3 gap-[70px]">${a}</ul>
  `,e}async function A(e){try{const s=await fetch(`${S}/products/${e}/`);if(!s.ok)throw new Error("상품 상세 데이터를 가져오는 데 실패했습니다.");return await s.json()}catch(s){throw console.error(s),s}}async function F(e){const s=document.createElement("div");try{const a=await A(e);s.className="product-detail max-w-container m-auto py-[80px]",s.innerHTML=`
