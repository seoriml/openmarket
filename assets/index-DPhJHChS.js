(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const c of o)if(c.type==="childList")for(const d of c.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&a(d)}).observe(document,{childList:!0,subtree:!0});function n(o){const c={};return o.integrity&&(c.integrity=o.integrity),o.referrerPolicy&&(c.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?c.credentials="include":o.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function a(o){if(o.ep)return;o.ep=!0;const c=n(o);fetch(o.href,c)}})();function I(){const e=JSON.parse(localStorage.getItem("userToken"));if(console.log(e),e&&e.user_type)switch(e.user_type){case"BUYER":return"buyer";case"SELLER":return"seller";default:return"guest"}else return"guest"}async function _(){try{if(!(await fetch(`${$}/accounts/logout/`,{method:"POST",headers:{"Content-Type":"application/json"}})).ok)throw new Error("로그아웃 요청에 실패했습니다.");localStorage.removeItem("userToken"),alert("로그아웃 되었습니다."),window.location.href="/"}catch(e){alert(e.message)}}function T(){const e=document.createElement("header");e.className="shadow-lg";const s=I(),n={guest:[{href:"#cart",text:"장바구니",icon:"/src/images/icon-shopping-cart.svg"},{href:"#login",text:"로그인",icon:"/src/images/icon-user.svg"}],buyer:[{href:"#cart",text:"장바구니",icon:"/src/images/icon-shopping-cart.svg"},{href:"#logout",text:"로그아웃",icon:"/src/images/icon-user.svg"}],seller:[{href:"#seller-center",text:"판매자 센터",icon:"/src/images/icon-shop.svg"},{href:"#logout",text:"로그아웃",icon:"/src/images/icon-user.svg"}]},a=({href:g,text:p,icon:u})=>`
    <a href="${g}" class="w-[50px] pt-[32px] bg-no-repeat bg-top text-center text-[12px] text-[#767676]" style="background:url('/openmarket/${u}') no-repeat center top">
      ${p}
    </a>
  ...
  `,s==="guest"){const g=e.querySelector('a[href="#cart"]');g&&g.addEventListener("click",p=>{p.preventDefault();const u=document.getElementById("loginModal");u.classList.remove("hidden");const m=document.getElementById("modalYes"),f=document.getElementById("modalNo"),h=document.getElementById("modalClose");m.addEventListener("click",()=>{sessionStorage.setItem("beforePage",window.location.hash),window.location.hash="login"}),f.addEventListener("click",()=>{u.classList.add("hidden")}),h.addEventListener("click",()=>{u.classList.add("hidden")})})}const c=e.querySelector('a[href="#login"]');c&&c.addEventListener("click",g=>{g.preventDefault(),sessionStorage.setItem("beforePage",window.location.hash),window.location.hash="login"});const d=e.querySelector('a[href="#logout"]');return d&&d.addEventListener("click",g=>{g.preventDefault(),_()}),e.querySelector(".search-form").addEventListener("submit",g=>{g.preventDefault();const p=e.querySelector("#search-input");console.log("검색어:",p.value)}),e}function j(){const e=document.createElement("footer");return e.className="bg-gray-100 py-8 px-4 border-t border-gray-200",e.innerHTML=`
  ...
  `;const a=e.querySelector("button.buyer"),o=e.querySelector("button.seller"),c=e.querySelector(".user-form");let d=a,y="BUYER";const g=(p,u)=>{const m=u.getAttribute("data-type").toLowerCase(),f=p.getAttribute("data-type");u.classList.contains("active")&&u.classList.remove("active"),!p.classList.contains("active")&&p.classList.add("active"),y=f,c.classList.contains(m)&&c.classList.remove(m),!c.classList.contains(f.toLowerCase())&&c.classList.add(f.toLowerCase()),d=p};return a.addEventListener("click",()=>{g(a,d)}),o.addEventListener("click",()=>{g(o,d)}),c.addEventListener("submit",async p=>{p.preventDefault();const u=e.querySelector("#userId").value.trim(),m=e.querySelector("#password").value.trim(),f=e.querySelector("#loginError");let h="";if(u===""&&m===""?h="아이디와 비밀번호를 입력해주세요.":u===""?h="아이디를 입력해주세요.":m===""&&(h="비밀번호를 입력해주세요."),h){f.textContent=h,f.style.display="block",u===""?e.querySelector("#userId").focus():m===""&&e.querySelector("#password").focus();return}const v=await fetch("https://openmarket.weniv.co.kr/accounts/login/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,password:m,login_type:y})});if(v.ok){const k=await v.json(),S={user_type:k.user_type,token:k.token,cart:[]};localStorage.setItem("userToken",JSON.stringify(S));const L=sessionStorage.getItem("beforePage");window.location.href=`/${L}`}else f.textContent="아이디 또는 비밀번호가 일치하지 않습니다.",f.style.display="block",m.value="",m.focus()}),e}const q=JSON.parse(localStorage.getItem("userToken")),w={async fetch(e,s={}){const n={headers:{"Content-Type":"application/json",...q&&{Authorization:`JWT ${q.token}`}}},a=await fetch(`${$}${e}`,{...n,...s});if(!a.ok)throw new Error(`API request failed: ${a.statusText}`);return a.json()},getCart:()=>w.fetch("/cart/"),getProduct:e=>w.fetch(`/products/${e}/`),updateCartItem:(e,s)=>w.fetch(`/cart/${e}/`,{method:"PUT",body:JSON.stringify(s)}),deleteCartItem:async e=>{const s=await fetch(`${$}/cart/${e}/`,{method:"DELETE",headers:{Authorization:`JWT ${q.token}`}});if(!s.ok)throw new Error(`API request failed: ${s.statusText}`);return s.status===204?null:s.json()}};async function B(){const e=document.createElement("section");e.className="max-w-container m-auto text-center";const s=await w.getCart();let n=await Promise.all(s.results.map(async t=>{const r=await w.getProduct(t.product_id);return{...t,product:r}}));function a(){return n.map(t=>`
        <!-- 장바구니 항목 시작 -->
      ...
      `).join("")}const o="w-1/4",c="w-[34px] h-[34px] bg-white bg-contain bg-center bg-no-repeat rounded-full",d="mb-[12px",y="text-[24px] font-bold",g="modal fixed inset-0 bg-black bg-opacity-50 hidden",p="border border-[#c4c4c4] py-[10px] w-[100px] rounded-[5px]",u="top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-[360px] text-center relative bg-white pt-[50px] pb-[40px] border border-[#767676]",m="w-[50px] h-[50px] bg-contain bg-center bg-no-repeat",f="absolute top-[18px] right-[18px] w-[22px] h-[22px] bg-[url('/src/images/icon-delete.svg')] bg-no-repeat bg-center bg-contain",h="w-[50px] h-[50px] border-x-[1px] border-[#c4c4c4] flex items-center justify-center text-center text-[18px] focus:outline-none";e.innerHTML=`
  ...
  <div id="quantity-modal" class="${g}">
    <div class="${u} absolute flex flex-col items-center">
      ...
      <div class="w-[150px] border border-[#c4c4c4] rounded-[10px] flex items-center justify-center">
        <button id="decrease-quantity" class="${m}" style="background-image: url('/openmarket/src/images/icon-minus-line.svg'); background-size: 20px;"></button>
        <input type="number" id="quantity-input" min="1" class="${h}" />
        <button id="increase-quantity" class="${m}" style="background-image: url('/openmarket/src/images/icon-plus-line.svg'); background-size: 20px;"></button>
      </div>
      <div class="flex justify-center gap-[10px] mt-[30px]">
  ...
  <!-- 삭제 확인 모달 -->
  <div id="delete-modal" class="class=${g}">
    <div class="${u}">
      <button id="modalClose" class="${f}"></button>   
...
`;const v=()=>{let t=0,r=0;n.forEach(i=>{e.querySelector(`div[data-id="${i.cart_item_id}"] .item-checkbox`).checked&&(t+=i.product.price*i.quantity,r+=i.product.shipping_fee)}),e.querySelector("#total-price").textContent=`${t.toLocaleString()}`,e.querySelector("#shipping-fee").textContent=`${r.toLocaleString()}`,e.querySelector("#final-price").textContent=`${(t+r).toLocaleString()}`},k=async(t,r)=>{const i=n.find(l=>l.cart_item_id===t);if(r>i.product.stock){alert("재고 수량을 초과했습니다.");return}try{await w.updateCartItem(t,{product_id:i.product.product_id,quantity:r,is_active:!0}),i.quantity=r;const l=e.querySelector(`div[data-id="${t}"]`);if(l){const x=l.querySelector(".quantity"),b=l.querySelector(".item-total-price");x&&(x.textContent=r),b?b.innerHTML=`
            ${(i.product.price*r).toLocaleString()} 원
          `:console.error("가격 표시 요소가 없습니다."),v()}else console.error("장바구니 항목을 찾을 수 없습니다.")}catch(l){console.error("수량 업데이트 실패:",l),alert("수량 업데이트에 실패했습니다.")}},S=async t=>{try{const r=await w.deleteCartItem(t);n=n.filter(l=>l.cart_item_id!==t);const i=e.querySelector(`div[data-id="${t}"]`);i&&i.remove(),v()}catch{alert("상품 삭제에 실패했습니다.")}},L=(t,r,i)=>{const l=e.querySelector("#quantity-modal"),x=l.querySelector("#quantity-input");x.value=r,x.max=i,l.style.display="block",l.querySelector("#increase-quantity").onclick=()=>{const b=parseInt(x.value)+1;b<=i&&(x.value=b)},l.querySelector("#decrease-quantity").onclick=()=>{const b=parseInt(x.value)-1;b>=1&&(x.value=b)},x.oninput=()=>{const b=parseInt(x.value);isNaN(b)||b<1?x.value=1:b>i&&(x.value=i)},l.querySelector("#quantity-confirm").onclick=()=>{k(t,parseInt(x.value)),l.style.display="none"},l.querySelector("#quantity-cancel").onclick=()=>{l.style.display="none"},l.querySelector("#modalClose").onclick=()=>{l.style.display="none"}},E=t=>{const r=e.querySelector("#delete-modal");r.style.display="block",r.querySelector("#delete-confirm").onclick=async()=>{await S(t),r.style.display="none"},r.querySelector("#delete-cancel").onclick=()=>{r.style.display="none"},r.querySelector("#modalClose").onclick=()=>{r.style.display="none"}};return e.querySelector(".cart-items").addEventListener("click",t=>{const r=t.target.closest("div[data-id]");if(!r)return;const i=parseInt(r.dataset.id),l=parseInt(r.querySelector(".quantity").textContent),x=parseInt(r.dataset.stock);t.target.classList.contains("increase-quantity")||t.target.classList.contains("decrease-quantity")&&l>1?L(i,l,x):t.target.classList.contains("delete-item")?E(i):t.target.classList.contains("item-checkbox")&&v()}),e.querySelector("#select-all").addEventListener("change",t=>{e.querySelectorAll(".item-checkbox").forEach(r=>r.checked=t.target.checked),v()}),v(),e}async function M(e){try{const s=await fetch(`${$}/products/${e}/`);if(!s.ok)throw new Error("상품 상세 데이터를 가져오는 데 실패했습니다.");return await s.json()}catch(s){throw console.error(s),s}}async function N(e){const s=document.createElement("div");try{const n=await M(e);s.className="product-detail max-w-container m-auto py-[80px]",s.innerHTML=`
