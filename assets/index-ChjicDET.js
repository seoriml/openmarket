(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))c(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const x of r.addedNodes)x.tagName==="LINK"&&x.rel==="modulepreload"&&c(x)}).observe(document,{childList:!0,subtree:!0});function a(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function c(o){if(o.ep)return;o.ep=!0;const r=a(o);fetch(o.href,r)}})();function B(){const e=JSON.parse(localStorage.getItem("userToken"));if(console.log(e),e&&e.user_type)switch(e.user_type){case"BUYER":return"buyer";case"SELLER":return"seller";default:return"guest"}else return"guest"}async function N(){try{if(!(await fetch(`${S}/accounts/logout/`,{method:"POST",headers:{"Content-Type":"application/json"}})).ok)throw new Error("로그아웃 요청에 실패했습니다.");localStorage.removeItem("userToken"),alert("로그아웃 되었습니다."),window.location.href="/openmarket"}catch(e){alert(e.message)}}function P(){const e=document.createElement("header");e.className="shadow-lg";const s=B(),a={guest:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#login",text:"로그인",icon:"icon-user.svg"}],buyer:[{href:"#cart",text:"장바구니",icon:"icon-shopping-cart.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}],seller:[{href:"#seller-center",text:"판매자 센터",icon:"icon-shop.svg"},{href:"#logout",text:"로그아웃",icon:"icon-user.svg"}]},c=({href:p,text:d,icon:u})=>`
  ...
  `,e}function z(){const e=document.createElement("div");e.className="flex flex-col items-center mt-[100px] mb-[350px]";const s="w-full py-[20px] pb-[32px] bg-[#F2F2F2] border border-[#c4c4c4] rounded-[10px]",a="w-full py-5 border-b border-gray-300 text-gray-700 outline-none";e.innerHTML=`
    <a href="/openmarket">
  ...
  `;const c=e.querySelector("button.buyer"),o=e.querySelector("button.seller"),r=e.querySelector(".user-form");let x=c,b="BUYER";const p=(d,u)=>{const m=u.getAttribute("data-type").toLowerCase(),g=d.getAttribute("data-type");u.classList.contains("active")&&u.classList.remove("active"),!d.classList.contains("active")&&d.classList.add("active"),b=g,r.classList.contains(m)&&r.classList.remove(m),!r.classList.contains(g.toLowerCase())&&r.classList.add(g.toLowerCase()),x=d};return c.addEventListener("click",()=>{p(c,x)}),o.addEventListener("click",()=>{p(o,x)}),r.addEventListener("submit",async d=>{d.preventDefault();const u=e.querySelector("#userId").value.trim(),m=e.querySelector("#password").value.trim(),g=e.querySelector("#loginError");let h="";if(u===""&&m===""?h="아이디와 비밀번호를 입력해주세요.":u===""?h="아이디를 입력해주세요.":m===""&&(h="비밀번호를 입력해주세요."),h){g.textContent=h,g.style.display="block",u===""?e.querySelector("#userId").focus():m===""&&e.querySelector("#password").focus();return}const $=await fetch("https://openmarket.weniv.co.kr/accounts/login/",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,password:m,login_type:b})});if($.ok){const w=await $.json(),y={user_type:w.user_type,token:w.token,cart:[]};localStorage.setItem("userToken",JSON.stringify(y));const q=sessionStorage.getItem("beforePage");window.location.href=`/openmarket/${q}`}else g.textContent="아이디 또는 비밀번호가 일치하지 않습니다.",g.style.display="block",m.value="",m.focus()}),e}async function O(){const e=JSON.parse(localStorage.getItem("userToken")),s={async fetch(t,n={}){const i={headers:{"Content-Type":"application/json",...e&&{Authorization:`JWT ${e.token}`}}},l=await fetch(`${S}${t}`,{...i,...n});if(!l.ok)throw new Error(`API request failed: ${l.statusText}`);return l.json()},getCart:()=>s.fetch("/cart/"),getProduct:t=>s.fetch(`/products/${t}/`),updateCartItem:(t,n)=>s.fetch(`/cart/${t}/`,{method:"PUT",body:JSON.stringify(n)}),deleteCartItem:async t=>{const n=await fetch(`${S}/cart/${t}/`,{method:"DELETE",headers:{Authorization:`JWT ${e.token}`}});if(!n.ok)throw new Error(`API request failed: ${n.statusText}`);return n.status===204?null:n.json()}},a=document.createElement("section");a.className="max-w-container m-auto text-center";const c=await s.getCart();let o=await Promise.all(c.results.map(async t=>{const n=await s.getProduct(t.product_id);return{...t,product:n}}));function r(){return o.map(t=>`
        <!-- 장바구니 항목 시작 -->
      ...
      `).join("")}const x="w-1/4",b="w-[34px] h-[34px] bg-white bg-contain bg-center bg-no-repeat rounded-full",p="mb-[12px",d="text-[24px] font-bold",u="modal fixed inset-0 bg-black bg-opacity-50 hidden z-50 ",m="border border-[#c4c4c4] py-[10px] w-[100px] rounded-[5px]",g="top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full max-w-[360px] text-center relative bg-white pt-[50px] pb-[40px] border border-[#767676]",h="w-[50px] h-[50px] bg-contain bg-center bg-no-repeat",$="absolute top-[18px] right-[18px] w-[22px] h-[22px] bg-[url('/images/icon-delete.svg')] bg-no-repeat bg-center bg-contain",w="w-[50px] h-[50px] border-x-[1px] border-[#c4c4c4] flex items-center justify-center text-center text-[18px] focus:outline-none";a.innerHTML=`
...
`;const y=()=>{let t=0,n=0;o.forEach(i=>{a.querySelector(`div[data-id="${i.cart_item_id}"] .item-checkbox`).checked&&(t+=i.product.price*i.quantity,n+=i.product.shipping_fee)}),a.querySelector("#total-price").textContent=`${t.toLocaleString()}`,a.querySelector("#shipping-fee").textContent=`${n.toLocaleString()}`,a.querySelector("#final-price").textContent=`${(t+n).toLocaleString()}`},q=async(t,n)=>{const i=o.find(l=>l.cart_item_id===t);if(n>i.product.stock){alert("재고 수량을 초과했습니다.");return}try{await s.updateCartItem(t,{product_id:i.product.product_id,quantity:n,is_active:!0}),i.quantity=n;const l=a.querySelector(`div[data-id="${t}"]`);if(l){const f=l.querySelector(".quantity"),v=l.querySelector(".item-total-price");f&&(f.textContent=n),v?v.innerHTML=`
            ${(i.product.price*n).toLocaleString()} 원
          `:console.error("가격 표시 요소가 없습니다."),y()}else console.error("장바구니 항목을 찾을 수 없습니다.")}catch(l){console.error("수량 업데이트 실패:",l),alert("수량 업데이트에 실패했습니다.")}},_=async t=>{try{const n=await s.deleteCartItem(t);o=o.filter(l=>l.cart_item_id!==t);const i=a.querySelector(`div[data-id="${t}"]`);i&&i.remove(),y()}catch{alert("상품 삭제에 실패했습니다.")}},E=(t,n,i)=>{const l=a.querySelector("#quantity-modal"),f=l.querySelector("#quantity-input");f.value=n,f.max=i,l.style.display="block",l.querySelector("#increase-quantity").onclick=()=>{const v=parseInt(f.value)+1;v<=i&&(f.value=v)},l.querySelector("#decrease-quantity").onclick=()=>{const v=parseInt(f.value)-1;v>=1&&(f.value=v)},f.oninput=()=>{const v=parseInt(f.value);isNaN(v)||v<1?f.value=1:v>i&&(f.value=i)},l.querySelector("#quantity-confirm").onclick=()=>{q(t,parseInt(f.value)),l.style.display="none"},l.querySelector("#quantity-cancel").onclick=()=>{l.style.display="none"},l.querySelector("#quantity-close").onclick=()=>{l.style.display="none"}},T=t=>{const n=a.querySelector("#delete-modal");n.style.display="block",n.querySelector("#delete-confirm").onclick=async()=>{await _(t),n.style.display="none"},n.querySelector("#delete-cancel").onclick=()=>{n.style.display="none"},n.querySelector("#delete-close").onclick=()=>{n.style.display="none"}};return a.querySelector(".cart-items").addEventListener("click",t=>{const n=t.target.closest("div[data-id]");if(!n)return;const i=parseInt(n.dataset.id),l=parseInt(n.querySelector(".quantity").textContent),f=parseInt(n.dataset.stock);t.target.classList.contains("increase-quantity")||t.target.classList.contains("decrease-quantity")?E(i,l,f):t.target.classList.contains("delete-item")?T(i):t.target.classList.contains("item-checkbox")&&y()}),a.querySelector("#select-all").addEventListener("change",t=>{a.querySelectorAll(".item-checkbox").forEach(n=>n.checked=t.target.checked),y()}),y(),a}async function H(){try{const e=await fetch(`${S}/products/`);if(!e.ok)throw new Error("상품 데이터를 가져오는 데 실패했습니다.");return(await e.json()).results}catch(e){return console.error(e),[]}}async function A(){const e=document.createElement("section");e.className="product-list-page max-w-container m-auto py-[80px]";const s=await H();if(s.length===0)return e.innerHTML="<p>상품이 없습니다.</p>",e;console.log(s);const a=s.map(c=>`
  ...
  `,e}async function D(e){try{const s=await fetch(`${S}/products/${e}/`);if(!s.ok)throw new Error("상품 상세 데이터를 가져오는 데 실패했습니다.");return await s.json()}catch(s){throw console.error(s),s}}async function F(e){const s=document.createElement("div");try{const a=await D(e);s.className="product-detail max-w-container m-auto py-[80px]",s.innerHTML=`
      ...
      `}catch(a){console.error("상품 상세 정보를 가져오는 데 실패했습니다.",a)}return s}function J(){const e=["https://via.placeholder.com/1200x500?text=Banner+1","https://via.placeholder.com/1200x500?text=Banner+2","https://via.placeholder.com/1200x500?text=Banner+3"];let s=0,a;function c(){const o=document.createElement("div");o.className="relative w-full h-full mx-auto bg-[#f2f2f2] max-h-[500px] overflow-hidden";const r=document.createElement("img");r.src=e[s],r.alt=`배너 ${s+1}`,r.className="w-full h-auto transition-opacity duration-500",o.appendChild(r);const x="absolute top-1/2 transform -translate-y-1/2 text-white px-4 py-2 focus:outline-none w-[8px] h-[16px] bg-no-repeat bg-center",b=document.createElement("button");b.className=`${x} bg-[url('/images/icon-swiper-1.svg')] left-4`,b.addEventListener("click",()=>{s=(s-1+e.length)%e.length,u(),h()});const p=document.createElement("button");p.className=`${x} bg-[url('/images/icon-swiper-2.svg')] right-4`,p.addEventListener("click",()=>{s=(s+1)%e.length,u(),h()}),o.appendChild(b),o.appendChild(p);const d=document.createElement("div");d.className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2",e.forEach(($,w)=>{const y=document.createElement("span");y.className=`w-[6px] h-[6px] rounded-full cursor-pointer transition-colors duration-300 ${w===s?"bg-black":"bg-white"}`,y.addEventListener("click",()=>{s=w,u(),h()}),d.appendChild(y)}),o.appendChild(d);function u(){r.src=e[s],r.alt=`배너 ${s+1}`,m()}function m(){d.querySelectorAll("span").forEach((w,y)=>{w.className=`w-2 h-2 rounded-full cursor-pointer transition-colors duration-300 ${y===s?"bg-black":"bg-white"}`})}function g(){a=setInterval(()=>{s=(s+1)%e.length,u()},3e3)}function h(){clearInterval(a),g()}return g(),o}return c()}const S="https://openmarket.weniv.co.kr",Y={login:z,cart:O,home:A,detail:F};async function I(){const e=document.getElementById("app");let s=!1,a="",c=window.location.hash.slice(1);c.includes("/")&&(a=c.split("/")[1],c=c.split("/")[0],s=!0),e.innerHTML="";try{const o=Y[c]||NotFound,r=s?await o(a):await o();if(c!=="login"){const b=P();e.appendChild(b)}if(c==="home"){const b=J();e.appendChild(b)}e.appendChild(r);const x=M();e.appendChild(x)}catch(o){console.error("페이지 렌더링 중 오류 발생:",o),e.innerHTML="<p>페이지를 로드하는 중 오류가 발생했습니다.</p>"}}function R(){window.addEventListener("hashchange",I),window.location.hash?I():window.location.hash="home"}window.addEventListener("load",R);
